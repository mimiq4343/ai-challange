...
- Branch `day-13` is created from `main`, adds the task state machine and disables personalization through `FEATURES.personalization` without deleting it.
...
- Main checks: targeted ESLint, `npx tsc --noEmit`, `npm run test:tasks`, `npm run test:profiles`, `npm run test:memory`, `npm run test:context-strategies`, `npm run test:compression`, `npm run test:tokens`, `npm run test:persistence`, and `npm run build` before integration.
