...
- Branch `day-12` is created from `main` and adds user profiles with profile-scoped long term memory and automatic preference learning.
...
- Main checks: targeted ESLint, `npx tsc --noEmit`, `npm run test:profiles`, `npm run test:memory`, `npm run test:context-strategies`, `npm run test:compression`, `npm run test:tokens`, `npm run test:persistence`, and `npm run build` before integration.
