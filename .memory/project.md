...
- Ветка `day-9` добавляет immutable summary checkpoints, compressed chat, operational savings и blind benchmark.
- Ветка `day-10` добавляет Sliding Window, Sticky Facts, persistent branching и единый benchmark без summary.
...
- Основные проверки: targeted ESLint, `npx tsc --noEmit`, `npm run test:context-strategies`, `npm run test:compression`, `npm run test:tokens`, `npm run test:persistence` и `npm run build` перед интеграцией.
