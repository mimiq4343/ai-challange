...
Day 13 добавляет STRICT-таблицы `task_runs`, `task_steps`, `task_events` и
маршруты автомата:
...
1. Завести задачу и попросить план — шаги появятся на этапе планирования.
2. Подтвердить план и закрыть первый шаг: агент перейдёт в `execution`.
3. Нажать «Пауза» и попросить агента двигаться дальше — состояние не изменится,
   в журнале появится `rejected`.
4. Снять паузу, открыть новый диалог и написать «Продолжаем»: агент продолжит с
   текущего шага без повторных объяснений.
5. Довести задачу до `done` и убедиться, что терминальный этап больше не
   принимает переходов.
