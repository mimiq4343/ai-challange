...
разрабатывается в отдельной ветке и вливается в `main` после проверки. Ветка
`day-11` создана от `day-8` и добавляет явную модель памяти агента: три слоя
хранятся раздельно, запись в них маршрутизируется явно, вклад каждого слоя в
промпт виден в интерфейсе.
...
3. Выключить тумблер LTM и повторить вопрос: агент отвечает, что не знает, а в
   телеметрии `LTM 0`.
4. Проверить, что `/day-7` и `/day-8` продолжают работать без изменений.
