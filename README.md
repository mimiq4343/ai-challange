...
Один и тот же запрос уходит по три раза при `temperature` 0, 0.7 и 1.2: повторы
...
разнообразие росло с температурой на всех задачах (на счётной 0% → 17% → 81%),
креативность метафоры поднялась с 2.0 до 4.0 из 5, а связность на сложной
символьной задаче упала с 5.0 до 2.3. Отдельная находка: `temperature: 0` не дал
побайтовой воспроизводимости — на одной из задач три ответа разошлись.
