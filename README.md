...
собраны дни с 1 по 13; ветка `day-14` добавляет инварианты: правила проекта, из-за
которых запрос может не дойти до модели. В `main` сейчас: измерение токенов, сжатие истории, стратегии
управления контекстом, модель памяти, персонализация и формализованное состояние
задачи — конечный автомат с этапами, шагами и ожидаемым действием. Персонализация
Day 12 выключена флагом `personalization`; код и данные профилей сохранены.
...
| Ветка   | Что в ней                                        |
| ------- | ------------------------------------------------ |
| `main`  | Стабильная версия приложения, сейчас Day 13      |
