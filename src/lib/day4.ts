// Конфигурация эксперимента Day 4: один запрос при четырёх температурах.
...
// Ограничение длины задаётся инструкцией, а не обрезкой по max_tokens: лимит
// токенов рубит ответ на полуслове, а модель, знающая рамку, укладывается в неё
// целиком. max_tokens остаётся страховкой от бесконечной генерации на высокой
// температуре и намеренно выставлен выше, чем нужно любому корректному ответу.
export const DAY4_SYSTEM = `Соблюдай ограничения по длине, заданные в запросе: если названо число слов, не превышай его.
Ответ должен быть законченным — не обрывай фразу и не оставляй мысль недосказанной. Если рискуешь не уложиться, скажи короче.
В любом случае не пиши длиннее 150 слов.`;

// Всё, кроме температуры, у четырёх колонок одинаковое: сравниваем один параметр.
