...
export const metadata: Metadata = {
  title: "Flash Chat · Day 16",
  description:
    "MCP-серверы: сохранение адресов, проверка соединения и просмотр инструментов рядом с агентом.",
};
...
export default async function Day16() {
  ...
  const invariants = getInvariantStore().getSnapshot(profile.id);
  const initialServers = getMcpServerStore().listServers(profile.id);
  const initialConversations = store.listConversations();
  const activeConversation = initialConversations[0] ?? null;
  const messages = activeConversation
    ? store.getMessages(activeConversation.id)
    : [];
        ...
        <main className="flex h-[calc(100dvh-5rem)] min-h-[680px] w-full flex-col gap-3">
          <div className="rise-in flex flex-col justify-between gap-2 sm:flex-row sm:items-end">
            ...
            <p className="max-w-[58ch] text-xs leading-relaxed text-muted sm:text-right">
              Агент с памятью, задачами и инвариантами. MCP-серверы и их
              инструменты — в скрываемой панели, без вызовов со стороны модели.
            </p>
          </div>
          <div className="min-h-0 flex-1">
            <Day15Workspace
              ...
              inspectorExtra={<McpServerPanel initialServers={initialServers} />}
            />
          </div>
        </main>
      </div>
